...
<head>
    ...
    <title>SIOPAL - {{ $title ?? 'Administrator' }}</title>
...
<body class="flex min-h-screen bg-background">
    ...
    <aside
        ...
        <nav class="flex-1 px-2 space-y-1">
            <a class="flex items-center rounded-lg px-4 py-3 mx-2 -mt-6 font-semibold transition-all {{ Request::is('admin/dashboard') ? 'bg-primary-container text-white hover:bg-primary' : 'text-on-surface-variant hover:bg-teal-50 hover:text-primary' }}"
                href="{{ route('admin.dashboard') }}">
                <span class="material-symbols-outlined mr-3">dashboard</span> Dashboard
            </a>

            <a class="flex items-center rounded-lg px-4 py-3 mx-2 font-semibold transition-all {{ Request::is('admin/manajemen-user*') ? 'bg-primary-container text-white hover:bg-primary' : 'text-on-surface-variant hover:bg-teal-50 hover:text-primary' }}"
                href="{{ route('admin.manajemen-user') }}">
                <span class="material-symbols-outlined mr-3">manage_accounts</span> User Management
            </a>

            <a class="flex items-center rounded-lg px-4 py-3 mx-2 font-semibold transition-all {{ Request::is('admin/audit-logs') ? 'bg-primary-container text-white hover:bg-primary' : 'text-on-surface-variant hover:bg-teal-50 hover:text-primary' }}"
                href="{{ route('admin.audit-logs') }}">
                <span class="material-symbols-outlined mr-3">history_edu</span> Audit Logs
            </a>
        </nav>
